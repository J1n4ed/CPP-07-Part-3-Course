...
namespace jinx
{
	...
	*/
		...
		*/
		...
		T get_value(std::string location)
		{
			...
			if (_content[location].has_value())
			{
				...
				{
					throw jinx::location_is_empty(location);
				}
			...
			else
			{
				...
				throw jinx::data_not_found(location, _content);
								...
								if (it_begin != raw_string.end()) // [ found before end of line
								{
									...
									if (it_end != raw_string.end())
									{
										...
										for (const auto& character : section_string)
										{
											if (character == ' ')
											{
												throw jinx::invalid_section_name(lineNum, section_string);
											}
										...
										std::map< std::string, std::map<std::string, jinx::DataBase& > >::iterator _mapIter;
									...
									{	
										...
										throw jinx::invalid_line(lineNum);
									}
								} // end if
								// !check if string is [section]
								...
								{	
									...
									if (isVar)
									{
										...
										for (const auto& character : currentVarName)
										{
											if (character == ' ')
											{
												throw jinx::invalid_variable_name(lineNum, currentVarName);
											}
										...
										*/
										...
										case -1:
										{
											...
											// jinx::DataBase* data_ptr = std::unique_ptr<DataBase>(new jinx::Data(std::optional<bool> no_val));
										...
										case 0:
										{
											...
											// jinx::DataBase * data_ptr = std::unique_ptr<DataBase>(new jinx::Data(currentVarValue));
											std::string key = currentSectioName + '.' + currentVarName;
										...
										case 1:
										{
											...
											// jinx::DataBase* data_ptr = std::unique_ptr<DataBase>(new jinx::Data(valueNum));
											std::string key = currentSectioName + '.' + currentVarName;
	...
	private:
		...
		// std::map< std::string, jinx::Data<std::any>> _content;
